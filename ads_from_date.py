...
def main():
    ...
    # 👉 приоритет: аргумент командной строки > .env > fallback today
    if len(sys.argv) > 1:
        date_start_str =  sys.argv[1]
    else:
        date_start_str = os.getenv("DATE_START", datetime.now().strftime('%Y-%m-%d'))

    try:
        last_saved_time = datetime.strptime(date_start_str, '%Y-%m-%d %H:%M:%S')
    except ValueError:
        print(f"[ERROR] Неверный формат даты: {date_start_str} (ожидается %Y-%m-%d")
        sys.exit(1)
    ...
    while True:
        batch = fetch_ads_batch(date1=next_start, date2=date2, city="Москва")#, source="5,6,7,10,11")
        if not batch:
            break
